...
echo -e "${GREEN}Deploying NeuraShield AI Service to GCP...${NC}"
...
if ! gcloud iam service-accounts describe "$SERVICE_ACCOUNT" --project="$PROJECT_ID" &> /dev/null; then
  ...
  gcloud iam service-accounts create "$SERVICE_NAME" \
    --display-name="NeuraShield AI Service Account" \
    --project="$PROJECT_ID" \
    --quiet
fi
...
gcloud builds submit --config cloudbuild-ai.yaml \
  --project="$PROJECT_ID" \
  --substitutions=_REGION="$REGION",_CPU="$CPU",_MEMORY="$MEMORY",_BLOCKCHAIN_API_URL="$BLOCKCHAIN_API_URL",_BLOCKCHAIN_ENABLED="$BLOCKCHAIN_ENABLED",_SERVICE_ACCOUNT="$SERVICE_ACCOUNT",_VPC_CONNECTOR="$VPC_CONNECTOR" \
  .
