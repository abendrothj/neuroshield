...
# Use the specific project ID instead of getting from gcloud config
...
# Create a Kubernetes secret for the service account
echo -e "${YELLOW}Creating Kubernetes secret for service account key...${NC}"
...
echo -e "${YELLOW}Applying GCP monitoring configuration...${NC}"
kubectl apply -f /home/jub/Cursor/neurashield/k8s/gcp-monitoring.yaml
...
rm -f key.json dashboard.json alert_policy.json
